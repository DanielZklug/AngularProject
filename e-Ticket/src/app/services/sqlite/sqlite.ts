...
export class Sqlite {
  ...
  public async addCoupon(coupon: Coupon) {
    const query = `
      INSERT INTO coupons (idcoupon, name, amount, create_at, status, type)
      VALUES (?, ?, ?, ?, ?, ?)
    `;
  ...
  public async updateCoupon(idcoupon: string) {
    const query = `UPDATE coupons SET status = 0 WHERE idcoupon = ? AND type = 'coupons'`;
    const result = await this.db.run(query, [idcoupon]);
  ...
  public async deleteCoupon(idcoupon: string) {
    const query = `DELETE FROM coupons WHERE idcoupon = ?`;
    const result = await this.db.run(query, [idcoupon]);
  ...
  public async filterCoupon(name: string) {
    const query = `SELECT * FROM coupons WHERE type = 'mycoupons' AND name = ?`;
